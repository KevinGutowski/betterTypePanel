...
function checkFontProps(textLayers) {
  framework("CoreText");
  var threadDictionary = NSThread.mainThread().threadDictionary();
  var font = textLayers[0].sketchObject.font();
  ...
  if (!featuresArray) {
    disableUI(threadDictionary, ['all']);
    sketch__WEBPACK_IMPORTED_MODULE_0___default.a.UI.message("⚠️ The selected font doesn't support the available font features with BetterTypePanel.");
    return;
  }
